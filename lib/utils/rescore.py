...
JOINT_BEAN_LINK_1 = [0, 1, 2, 3]
JOINT_BEAN_LINK_2 = [1, 2, 3, 4]
...
def get_feature(x, dataset):
    ...
    elif 'bean' in dataset:
        joint_1, joint_2 = JOINT_BEAN_LINK_1, JOINT_BEAN_LINK_2
    else:
        raise ValueError(
            'Please implement flip_index for new dataset: %s.' % dataset)

    # To get the Delta x Delta y
    joint_relate = joint_abs[:, joint_1] - joint_abs[:, joint_2]    # related distance
    joint_length = ((joint_relate**2)[:, :, 0] +
                    (joint_relate**2)[:, :, 1])**(0.5)      # joint length

    # To use the torso distance to normalize - 用躯干距离作为标准->该为1，2号豆粒间距离
    normalize = (joint_length[:, 0]+joint_length[:, 1])/2
