...
def down_interpolate(x,size,mode='bilinear'):
    H=x.size()[2]
    W=x.size()[3]
    scale_h=int(H/size[0])
    scale_w=int(W/size[1])
    x = x[:,:,0:H-scale_h+1, 0:W-scale_w+1]
    inter_x= torch.nn.functional.interpolate(x,size=size,align_corners=True,mode='bilinear')
    return inter_x
