...
def build_transforms(cfg, is_train_or_val=True):
    assert is_train_or_val is True, 'Please only use build_transforms for training / val.'

    if 'coco' in cfg.DATASET.DATASET:
        dataset_name = 'COCO'
    elif 'crowd_pose' in cfg.DATASET.DATASET:
        dataset_name = 'CROWDPOSE'
    elif 'bean' in cfg.DATASET.DATASET:
        dataset_name = 'BEAN'
    else:
