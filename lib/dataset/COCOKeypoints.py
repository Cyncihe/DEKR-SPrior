...
import pycocotools
from .COCODataset import CocoDataset, BeanDataset
...
class BeanKeypoints(BeanDataset):
    def __init__(self, cfg, dataset, heatmap_generator=None, offset_generator=None, graph_generator=None, transforms=None):
        super().__init__(cfg, dataset)
        self.num_joints = cfg.DATASET.NUM_JOINTS
        self.num_joints_with_center = self.num_joints + 1
    ...
    def __getitem__(self, idx):
        ...
        group_mask = self.graph_generator(img, anno)

        return img, heatmap, mask, offset, offset_weight, group_mask
