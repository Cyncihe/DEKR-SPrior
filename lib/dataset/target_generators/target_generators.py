...
class GraphGenerator():
    ...
    def __call__(self, img, anns):
        c, h, w = img.shape
        groupmap = - np.ones((int(h / self.stride), int(w / self.stride)))
        for n, ann in enumerate(anns):
            single_keypoints = np.array(ann['keypoints']).reshape(-1, 3)

            for i, p in enumerate(single_keypoints):
                if p[2] < 1 or p[0] < 0 or p[1] < 0 or p[0] > w or p[1] > h:
                    continue
                p[0:2] = p[0:2] / self.stride
                p = list(map(int, p))
                try:
                    groupmap[p[1]][p[0]] = n + 1
                except Exception as e:
                    print(e)
                    # print('in image:', anns['image_name'])
