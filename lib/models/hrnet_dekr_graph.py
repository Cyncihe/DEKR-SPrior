...
class GraphNet(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=128):
        ...
        self.trunk = nn.Sequential(
            ...
            AdaptBlock(mid_channels, mid_channels),
            Conv(mid_channels, out_channels, 3),
            Conv(out_channels, out_channels, 3)
        )

    def forward(self, x):
        graph_features = self.trunk(x)
...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.graph = GraphNet(480, 128, mid_channels=128)  # todo: modify input channel
    ...
    def forward(self, x):
        ...
        x = torch.cat([y_list[0], F.interpolate(y_list[1], size=(x0_h, x0_w), mode='bilinear'),
        ...
        return heatmap, offset, graph_feature
