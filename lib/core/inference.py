...
from lib.utils.transforms import up_interpolate
...
def get_reg_poses(offset, num_joints):
    ...
    locations = get_locations(h, w, offset.device)  # (16384, 2)
    locations = locations[:, None, :].expand(-1, num_joints, -1)     # (16384, 5, 2)
...
def offset_to_pose(offset, flip=True, flip_index=None):
    num_offset, h, w = offset.shape[1:]  # (1, 10, 128, 128)
    num_joints = int(num_offset / 2)        # 5
    reg_poses = get_reg_poses(offset[0], num_joints)    # has neg
    if flip:
...
def get_multi_stage_outputs(cfg, model, image, with_flip=False):
    # model output heatmap and offset
    if 'graph' in cfg.MODEL.NAME:
        heatmap, offset, _ = model(image)
    else:
